...
namespace NinjaTrader.Strategy
{
	...
	/// 	
    /// </summary>
    [Description("5 min CL bars - Ben's morning trade")]
    public class KeyIdea19 : Strategy
    {
        #region Variables
...
//		private bool is2nd = false;
		...
		rcVolatilityBreakout vbo = null;
		bool enableTrade = false;
		int squeezeSize = 0;
		int maxSinceSqueeze = 15;
		double squeezeHigh = 0;
		double squeezeLow = 0;
...
//				prevHigh = High[HighestBar(High, 3)];
			...
			if (vbo.BarsSinceSqueeze[0] > maxSinceSqueeze)
			{
				squeezeSize = 0;
				enableTrade = false;
			}
			...
			{
				squeezeHigh = vbo.SqueezeHigh[0];
				squeezeLow = vbo.SqueezeLow[0];
				enableTrade = true;			
			}
			...
			if (isFlat() && enableTrade)
			{
				...
				// short trade
				else if (atr.Lower.ContainsValue(0)
					&& squeezeSize > 5)
				{
					SetStopLoss(CalculationMode.Price, squeezeHigh);					
					
					if (Close[0] > squeezeLow)
					{
						EnterShortStop(squeezeLow - TickSize);
						SetProfitTarget(CalculationMode.Price, squeezeLow - TickSize - (squeezeHigh - squeezeLow));
					}
					else
					{						
						EnterShort();				
						SetProfitTarget(CalculationMode.Price, Close[0] - (squeezeHigh - squeezeLow));
					}
				}
			}
