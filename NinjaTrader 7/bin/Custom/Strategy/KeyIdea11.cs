...
#region Notes
/*
 This strategy has just started to be written, the code is a cut from KeyIdea09.cs
 
 The idea of KeyIdea11 is to use the HMA(10) indicator and the SMA(40) indicator with a longer period.  Trend will be
 determined from SMA slope and when to enter is when HMA changes color into the trend.  For example, in a downtrend, SMA falling,
 when HMA goes from green to red, sell market. 

 It would be good to add a trend strength filter, but not sure what would work good for that. Maybe I look back x bars for at least x ticks.
 
 I was experimenting this strategy using ES on a 1024 tick chart.
...
namespace NinjaTrader.Strategy
{
	...
	/// 	
    /// </summary>
    [Description("ES 1024 tick")]
    public class KeyIdea11 : Strategy
    {
        #region Variables
        ...
        private int iparm2 = 8; // Default setting for Stop
        ...
        private int period2 = 40; // Default setting for Period2
        private int period3 = 15; // Default setting for Period2

		private int startTime = 0;  // 850  start of trading hhmm
		private int stopTime = 2300;  // 1230 end of trading hhmm
...
//			if (5 == 7)
			...
			// ============================================
			//if (Falling(sma) && hma.TrendSet[0] == -1 && buyOrder1 == null) //  downtrend
			if (5 == 7)
			{
				if (Qty1 > 0)
				{
