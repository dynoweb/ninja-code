...
namespace NinjaTrader.Strategy
{
	...
	/// 	
    /// </summary>
    [Description("CL 5 min bars - Based on the book The Layman's Guide To Trading - TKO Trade, page 52")]
    public class TLGTTS_TKO : Strategy
    {
        #region Variables
...
//        private int buyAndHold = 0;
        ...
        private int iparm1 = 10;
		private int iparm2 = 50; // Default setting for Iparm2
...
//			}
		...
		private bool isStrongUpTrend()
		{
			// means in an up trend
			...
			if (ema[0] < hma[0])
			{
				if (!CrossBelow(ema, hma, 4))
				{
					return false;
				}
			}
			DrawDot(CurrentBar + "SDT",  false, 0, hma[0], Color.Green);			
						
			return true;
		}

		private bool isStrongDownTrend()
		{
			// means in a down trend
			...
			if (ema[0] > hma[0])
			{
				if (!CrossAbove(ema, hma, 4))
				{
					return false;
				}
			}
			DrawDot(CurrentBar + "SDT",  false, 0, hma[0], Color.Red);			
		...
		/// <summary>
		/// To qualify for a bear trade, 
		/// 	Downtrend - must be strong and persistent
		///  	Pullback/Knockout bar - close above prior two bar highs
		/// 	Bonus - ideally the trigger bar should be wide range bar (WRB) to remove weak sellers
		/// 
		/// </summary>
		/// <returns></returns>
		private bool isBear()
...
//        }
        ...
        {
            get { return atrTimes; }
            set { atrTimes = Math.Max(0.000, value); }
        }
