...
namespace NinjaTrader.Indicator
{
	...
	public class MACDRicksCopy : Indicator
	{
		#region Variables
		private int					fast	= 5;
		private int					slow	= 20;
...
namespace NinjaTrader.Indicator
{
    ...
    {
        private MACDRicksCopy[] cacheMACDRicksCopy = null;

        private static MACDRicksCopy checkMACDRicksCopy = new MACDRicksCopy();

        /// <summary>
        /// The MACD (Moving Average Convergence/Divergence) is a trend following momentum indicator that shows the relationship between two moving averages of prices.
        /// </summary>
        /// <returns></returns>
        public MACDRicksCopy MACDRicksCopy(int fast, int slow, int smooth)
        {
            return MACDRicksCopy(Input, fast, slow, smooth);
        }
        ...
        /// </summary>
        /// <returns></returns>
        public MACDRicksCopy MACDRicksCopy(Data.IDataSeries input, int fast, int slow, int smooth)
        {
            if (cacheMACDRicksCopy != null)
                for (int idx = 0; idx < cacheMACDRicksCopy.Length; idx++)
                    if (cacheMACDRicksCopy[idx].Fast == fast && cacheMACDRicksCopy[idx].Slow == slow && cacheMACDRicksCopy[idx].Smooth == smooth && cacheMACDRicksCopy[idx].EqualsInput(input))
                        return cacheMACDRicksCopy[idx];

            lock (checkMACDRicksCopy)
            {
                checkMACDRicksCopy.Fast = fast;
                fast = checkMACDRicksCopy.Fast;
                checkMACDRicksCopy.Slow = slow;
                slow = checkMACDRicksCopy.Slow;
                checkMACDRicksCopy.Smooth = smooth;
                smooth = checkMACDRicksCopy.Smooth;
                ...
                MACDRicksCopy indicator = new MACDRicksCopy();
                ...
                MACDRicksCopy[] tmp = new MACDRicksCopy[cacheMACDRicksCopy == null ? 1 : cacheMACDRicksCopy.Length + 1];
                if (cacheMACDRicksCopy != null)
                    cacheMACDRicksCopy.CopyTo(tmp, 0);
                tmp[tmp.Length - 1] = indicator;
                cacheMACDRicksCopy = tmp;
                return indicator;
            }
        }
    }
...
namespace NinjaTrader.MarketAnalyzer
{
    public partial class Column : ColumnBase
    {
        ...
        public Indicator.MACDRicksCopy MACDRicksCopy(int fast, int slow, int smooth)
        {
            return _indicator.MACDRicksCopy(Input, fast, slow, smooth);
        }
        ...
        /// </summary>
        /// <returns></returns>
        public Indicator.MACDRicksCopy MACDRicksCopy(Data.IDataSeries input, int fast, int slow, int smooth)
        {
            return _indicator.MACDRicksCopy(input, fast, slow, smooth);
        }
    }
