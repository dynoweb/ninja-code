...
namespace NinjaTrader.Strategy
{
	...
	/// 	
    /// </summary>
    [Description("ES 5 min primary with 10 sec secondary")]
    public class KeyIdea09m : Strategy
    {
        #region Variables
...
//			}
				...
				// ============================================
				//if (HMARick(BarsArray[1], Period1, 200).TrendSet[0] == 1 && sellOrder1 == null) //  uptrend
				if (isGoLong() && sellOrder1 == null)
				{
					if (Qty1 > 0)
					{
					...
					if (Qty2 > 0)
					{
						limitPrice = Lows[0][0] - (EntryB1 + EntryB2) * TickSize;
						stopPrice = limitPrice;
						...
						{
							buyOrder2 = SubmitOrder(1, OrderAction.Buy, OrderType.Limit, Qty2, limitPrice, stopPrice, orderPrefix + "oco2", "B2");
						}
					}
					...
					if (Qty3 > 0)
					{
						limitPrice = Lows[0][0] - (EntryB1 + EntryB2 + EntryB3) * TickSize;
						stopPrice = limitPrice;
						...
						{
							buyOrder3 = SubmitOrder(1, OrderAction.Buy, OrderType.Limit, Qty3, limitPrice, stopPrice, orderPrefix + "oco3", "B3");
						}
					}
		...
		protected override void OnExecution(IExecution execution)
		{
			...
			if (buyOrder2 != null && buyOrder2 == execution.Order)
			{
				if (closeLongOrderLimit2 == null) 
				{
					...
					closeLongOrderStop2 = SubmitOrder(1, OrderAction.Sell, OrderType.Stop, execution.Order.Quantity, 
						limitPrice, stopPrice, orderPrefix + "ocoCloseB2", "CSB2");

					stopPrice = 0;
					limitPrice = buyOrder2.AvgFillPrice + (target + EntryS2) * TickSize;
					closeLongOrderLimit2 = SubmitOrder(1, OrderAction.Sell, OrderType.Limit, execution.Order.Quantity, 
			...
			if (buyOrder3 != null && buyOrder3 == execution.Order)
			{
				if (closeLongOrderLimit3 == null) 
				{
					...
					closeLongOrderStop3 = SubmitOrder(1, OrderAction.Sell, OrderType.Stop, execution.Order.Quantity, 
						limitPrice, stopPrice, orderPrefix + "ocoCloseB3", "CSB3");

					stopPrice = 0;
					limitPrice = buyOrder3.AvgFillPrice + (target + EntryS2 + EntryS3) * TickSize;
					closeLongOrderLimit3 = SubmitOrder(1, OrderAction.Sell, OrderType.Limit, execution.Order.Quantity, 
			...
			if (sellOrder2 != null && sellOrder2 == execution.Order)
			{
				if (closeShortOrderLimit2 == null) 
				{
					...
					closeLongOrderStop2 = SubmitOrder(1, OrderAction.BuyToCover, OrderType.Stop, execution.Order.Quantity, 
						limitPrice, stopPrice, orderPrefix + "ocoCloseS2", "CSS2");

					stopPrice = 0;
					limitPrice = sellOrder2.AvgFillPrice - (target + EntryS2) * TickSize;
					closeShortOrderLimit2 = SubmitOrder(1, OrderAction.BuyToCover, OrderType.Limit, execution.Order.Quantity, 
			...
			if (sellOrder3 != null && sellOrder3 == execution.Order)
			{
				if (closeShortOrderLimit3 == null) 
				{
					...
					closeLongOrderStop3 = SubmitOrder(1, OrderAction.BuyToCover, OrderType.Stop, execution.Order.Quantity, 
						limitPrice, stopPrice, orderPrefix + "ocoCloseS3", "CSS3");

					stopPrice = 0;
					limitPrice = sellOrder3.AvgFillPrice - (target + EntryS2 + EntryS3) * TickSize;
					closeShortOrderLimit3 = SubmitOrder(1, OrderAction.BuyToCover, OrderType.Limit, execution.Order.Quantity, 
...
//				}
			...
			if (buyOrder2 != null)
			{
				if (buyOrder2.OrderState == OrderState.Filled && closeLongOrder2 == null)
				{
					if (ToTime(Time[0]) > (StopTime + 15) * 100)	// wait 15 more min before closing
					{
						closeLongOrder2 = SubmitOrder(1, OrderAction.Sell, OrderType.Market, buyOrder2.Quantity, 
			...
			if (buyOrder3 != null)
			{
				if (buyOrder3.OrderState == OrderState.Filled && closeLongOrder3 == null)
				{
					if (ToTime(Time[0]) > (StopTime + 15) * 100)	// wait 15 more min before closing
					{
						closeLongOrder3 = SubmitOrder(1, OrderAction.Sell, OrderType.Market, buyOrder3.Quantity, 
			...
			if (sellOrder2 != null)
			{
				if (sellOrder2.OrderState == OrderState.Filled && closeLongOrder2 == null)
				{
					if (ToTime(Time[0]) > (StopTime + 15) * 100)	// wait 15 more min before closing
					{
						closeLongOrder2 = SubmitOrder(1, OrderAction.BuyToCover, OrderType.Market, sellOrder2.Quantity, 
			...
			if (sellOrder3 != null)
			{
				if (sellOrder3.OrderState == OrderState.Filled && closeLongOrder3 == null)
				{
					if (ToTime(Time[0]) > (StopTime + 15) * 100)	// wait 15 more min before closing
					{
						closeLongOrder3 = SubmitOrder(1, OrderAction.BuyToCover, OrderType.Market, sellOrder3.Quantity, 
        ...
        [Description("ATR Trailing Period")]
        [GridCategory("Parameters")]
        public int Period3
        {
            get { return period3; }
