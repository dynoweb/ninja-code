...
namespace NinjaTrader.Strategy
{
	...
	/// 
	/// Revision History
	///  v8a - 9/14 2am - added close of position when hull turns 
	/// 
	/// Ref
	/// http://www.solerinvestments.com/Online-Trading/Stock-Market-Order-Types.htm
	/// 
	/// Testing
	/// - Replay testing from 11/17/2012 - 1/12/2013 - 8,0,55,true,true,true,false,true,true,false,68,35,90,12,0,7,16,12,25,19
    ...
    public class TATrader : Strategy
    {
		...
		// Algo RTM Long likes ATR 3x,18p, 0.002r and Donchian 34p,
		// v4 RTM 377T $14,610 - 18, .005, 11, 48, 58, 35, 115, 15, 40, 0, 16, 0, 0 
		// v4 MOMO 377T $15,395 10, 0.001, 10, 55, T, F, 58, 35, 90, 14, 40, 0, 1, 25, 19, 1
		// approx v7 All 377t $11,865 (may 1st-sep 6) 8,0,10,55, f, t, t, t, 58, 35, 90, 14, 0, 7, 19, 8, 30, 26
		// v7 all 377t $18,700 (may 1st-sep 6) 8,0,10,55, f, t, t, t, t, t, 58, 35, 90, 12, 0, 8, 15, 7, 28, 24
        ...
        private int donchianPeriod = 55; //20 Default setting for DonchianPeriod Optimized MOMO = 22
        ...
        private int hullPeriod = 68; // Default setting for HullPeriod Optimized MOMO = Didn't really matter
		...
		// ATM Stop Stategy Variables - for YTD
		// v8 Slo-Mo short $4,940.0 - PartialAdj 5, ABEProfitTrigger 20, ProfitTrigger1 11, ProfitTrigger 33, StopLoss 20
		// v8 Slo-Mo long  $9,265.0 - PartialAdj 5, ABEProfitTrigger 15, ProfitTrigger1  9, ProfitTrigger 45, StopLoss 22
		// v8 Slo-Mo both $12,860.0 - PartialAdj 5, ABEProfitTrigger 15, ProfitTrigger1 11, ProfitTrigger 48, StopLoss 20
		// v8a Slo-Mo short  -$640.0 - HullP 68, PartialAdj  5, ABEProfitTrigger 15, ProfitTrigger1  9, ProfitTrigger 35, StopLoss 29
		// v8a Slo-Mo short $6,320.0 - HullP 68, PartialAdj 19, ABEProfitTrigger 19, ProfitTrigger1  7, ProfitTrigger 46, StopLoss 29, maxdd -4,440
		// v8a Slo-Mo long  $7,295.0 - HullP 68, PartialAdj  5, ABEProfitTrigger 15, ProfitTrigger1  9, ProfitTrigger 35, StopLoss 29, maxdd -2,465
		// v8b Momo both 
        ...
        protected override void Initialize()
        {
			...
			DonchianChannel(DonchianPeriod).Displacement = 2;
			DonchianChannel(DonchianPeriod).AutoScale = false;
			DonchianChannel(DonchianPeriod).Plots[0].Pen.Color = Color.DarkGreen;	// mean
			DonchianChannel(DonchianPeriod).Plots[1].Pen.Color = Color.DarkRed;		// top plot
			DonchianChannel(DonchianPeriod).Plots[2].Pen.Color = Color.DarkRed;		// bottom plot
			DonchianChannel(DonchianPeriod).PaintPriceMarkers = false;
			
            //if (enableDebug) Add(MACDrick(mACDFast, mACDSlow, mACDSmooth));
            if (enableDebug)
			{				
				Add(ATRTrailing(ATRTimes, ATRPeriod, ATRRatched));
				ATRTrailing(ATRTimes, ATRPeriod, ATRRatched).AutoScale = false;
				ATRTrailing(ATRTimes, ATRPeriod, ATRRatched).PaintPriceMarkers = false;
				
				Add(HMARick(HullPeriod, 30));
				HMARick(HullPeriod, 30).PaintPriceMarkers = false;
			}
...
//				return;
		...
		if (EnableMomo) 
		{
			...
			if (isFlat()
				...
				&& isAlgoStopPos()
				&& isMacdPos()
				&& Low[0] > DonchianChannel(DonchianPeriod).Upper[disp]
				&& Close[0] > Open[0]
				&& enableLong
				)
			{
			...
			if (isFlat()
				...
				&& isAlgoStopNeg()
				&& isMacdNeg()
				&& High[0] < DonchianChannel(DonchianPeriod).Lower[disp]
				&& enableShort
				)
			{
	...
	if (EnableSloMo) 
	{
		if (isFlat())
		{
			switch (sloMoFlag) {
				...
				case 0: {
					if (High[0] > DonchianChannel(DonchianPeriod).Upper[disp]
						&& enableLong
						) 
					{
					...
					if (Low[0] < DonchianChannel(DonchianPeriod).Lower[disp]
						&& enableShort
						) 
					{
				...
				case 1: {
					...
					if (High[0] + 2*TickSize < DonchianChannel(DonchianPeriod).Upper[disp]) {
						if (momoFlag == 0
							&& isAlgoBarPos()
							&& isAlgoStopPos()
							&& isMacdPos()
						...
						{
							sloMoFlag = 2;
							//EnterLongStopLimit(DefaultQuantity, DonchianChannel(DonchianPeriod).Upper[disp], DonchianChannel(DonchianPeriod).Upper[disp], "Slo-Mo Long");

							//if (enableDebug) DrawArrowUp(CurrentBar.ToString()+"slomo", 0, DonchianChannel(DonchianPeriod).Upper[disp] + 10 * TickSize, Color.LightBlue);
				...
				case -1: {
					...
					if (Low[0] - 2*TickSize > DonchianChannel(DonchianPeriod).Lower[disp]) {
						if (momoFlag == 0
							&& isAlgoBarNeg()
							&& isAlgoStopNeg()
							&& isMacdNeg()
				...
				case 2: {
					...
					if (Low[0] < DonchianChannel(DonchianPeriod).Mean[disp]
						|| DonchianChannel(DonchianPeriod).Upper[disp+15] >= DonchianChannel(DonchianPeriod).Upper[disp] 
						)
					{							
					...
					{
						slmoEntry = DonchianChannel(DonchianPeriod).Upper[disp];			
						...
						*/
						...
						//if (enableDebug) DrawArrowUp(CurrentBar.ToString()+"slomo", 0, DonchianChannel(DonchianPeriod).Upper[disp] + 10 * TickSize, Color.Blue);
					}
				...
				case -2: {
					if (High[0] > DonchianChannel(DonchianPeriod).Mean[disp]
						|| DonchianChannel(DonchianPeriod).Lower[disp+15] <= DonchianChannel(DonchianPeriod).Lower[disp] 
						)
					{							
					...
					{
						slmoEntry = DonchianChannel(DonchianPeriod).Lower[disp];							
		...
		if (enableDebug) {
			if (sloMoFlag > 0)
				DrawText(CurrentBar.ToString()+"ESM", sloMoFlag+"", 0, DonchianChannel(DonchianPeriod).Lower[disp] + 5 * TickSize, Color.Black);
			if (sloMoFlag < 0)
				DrawText(CurrentBar.ToString()+"ESM", sloMoFlag+"", 0, DonchianChannel(DonchianPeriod).Upper[disp] - 5 * TickSize, Color.Black);
		}
	} // end if EnableSloMo		
		#endregion			
		...
		if (enableRtm) 
		{
			if (isFlat())
			{
				switch (rtmFlag) {
					...
					case 0: {
						if (High[0] > DonchianChannel(DonchianPeriod).Upper[disp]
							&& Low[0] > DonchianChannel(DonchianPeriod).Mean[disp]
							&& enableLong
							) 
						{
							if (enableDebug) BackColorAll = Color.Cyan;
					...
					case 1: {
						...
						if (DonchianChannel(DonchianPeriod).Mean[disp + minFlatMeanBars] >= DonchianChannel(DonchianPeriod).Mean[disp]
							&& isAlgoBarPos()
							&& isAlgoStopPos()
							&& isMacdPos()
							)
						{
							...
							if (momoFlag == 0 && sloMoFlag == 0) {
								rtmEntry = DonchianChannel(DonchianPeriod).Mean[disp];
								//EnterLongLimit(DefaultQuantity, rtmEntry, "RTM Limit Long");
								order.isLong = true;
								order.limitPrice = rtmEntry;
								order.type = "l";
								order.signalName = "RTM Limit Long";
								if (enableDebug) DrawArrowUp(CurrentBar.ToString()+"rtm", 0, DonchianChannel(DonchianPeriod).Mean[disp] - 10 * TickSize, Color.Green);
							} else if (Low[0] < DonchianChannel(DonchianPeriod).Mean[disp]) {
			...
			if (enableDebug) {
				if (rtmFlag > 0) 
					DrawText(CurrentBar.ToString()+ "1",  "1", 0, DonchianChannel(DonchianPeriod).Mean[disp] + 5 * TickSize, Color.Blue);
				if (rtmFlag < 0)					
					DrawText(CurrentBar.ToString()+"-1", "-1", 0, DonchianChannel(DonchianPeriod).Mean[disp] - 5 * TickSize, Color.Red);
			}
		}			
		#endregion
