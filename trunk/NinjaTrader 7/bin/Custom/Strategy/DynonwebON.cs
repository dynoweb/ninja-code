...
namespace NinjaTrader.Strategy
{
    ...
    public class DynowebON : Strategy
    {
        #region Variables
...
//
        ...
        protected override void OnBarUpdate()
        {
			...
			// Run on the trigger bar to calculate daily rangecheck
			if (adjustedChannelSize == 0)
			{
				if (Bars.BarsSinceSession > period && Bars.BarsSinceSession < (500 + period))
				//if (ToTime(Time[0]) == ToTime(hour, minute, 0)
				//	|| (extendPeriod == 1
				//		&& ToTime(Time[0]) >= ToTime(hour, minute, 0))
				//		&& ToTime(Time[0]) <= ToTime(hour, minute + BarsPeriod.Value * period, 0))
				//	)
				{
		...
		/// <summary>
		/// Calculate the width of the normal trade range 
		/// </summary>
		/// <returns></returns>
		private int calcAdjustedChannelSize()
		{
			int checkPeriod = (Bars.BarsSinceSession - period);  // Bars.PercentComplete
			//Print(Time + " checkPeriod: " + checkPeriod + " Bars.BarSinceSession: " + Bars.BarsSinceSession);
			...
			//Print(Time + " sessionLow: " + sessionLow + " sessionHigh: " + sessionHigh);
		/*
			...
			*/
			if (adjustedChannelSize != 0)
			{
				...
				DrawText(CurrentBar + "adj", adjustedChannelSize.ToString(), 10, channelHigh + 5 * TickSize, Color.Black);
